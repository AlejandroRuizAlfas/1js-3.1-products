...
class Store {
    ...
    addCategory(nombre, descripcion){
        //TODO COmprobar si se pasa descripción o no
        if (!nombre){
            throw "No existe una categoria con ese nombre"
        }
        // TRY CATCH
    ...
    //TODO no se le pasa un nombre / ya existe una categoría con ese nombre
    addProduct(prod){
        if(!prod.name){
        ...
        // }
        // if(!this.categories.find(cat => cat.id === prod.id)){
        //     throw "No puedes crear un producto en una categoria que no"
        // }
